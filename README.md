...
### Exporting Data
Click the **Download** button in the header to export all graph data as a JSON file. This creates a backup you can store or share.

### Importing Data
Click the **Upload** button in the header to import data from a JSON file. The import:
- Validates the JSON structure before importing
- Adds new nodes and relationships to existing data
- Shows a success message with the count of imported items
...
```json
{
  ...
  "edges": [
    {
      "source_node": "unique-id",
      "target_node": "project-id",
...
**Required fields for nodes:** `node_id`, `node_type`, `display_name`
**Required fields for edges:** `source_node`, `target_node`, `relationship_type`

## Syncing Code Between Replit and Local

After making changes in Replit, download the latest code before running locally:

1. In Replit, click the **three dots menu** at the top left
2. Select **"Download as zip"**
3. Extract and replace your local project folder
